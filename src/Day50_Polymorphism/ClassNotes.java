...
Polymorphism: many forms
                WHEN PARENT/CLASS INTERFACE IS REFERENCED TO THE CHILDS OBJECT

                - whatever methods from the parent class that were OVERRIDDEN, that is what will be printed/executed
                - the only time the method in Object gets executed, is IF it was overridden. If a method was not executed it will print
                    the reference class method

instanceOf - can be used to check if the object is in certain classes
           - identifies if the referenceName is referenced to the classes object
           if(a instanceof Dog) = if a is referenced to dogs object then it will output TRUE OR FASLE
                                    -
      OOP Summary
   ...
   Abstraction: hiding the implementation details
         ...
         there are ONLY two ways to achieve abstraction:
                ...
                2. interface: we can implement multiple interfaces to a class ( cannot be final)
                              everything has public access modifer by default
                              we can only have:
                                            varaibles: by default static and final
